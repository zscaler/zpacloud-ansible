...
class TestZPAPolicyAccessIsolationRuleV2Module(ModuleTestCase):
    ...
    def test_update_rule(self, mock_client, mocker):
        ...
        mocker.patch("ansible_collections.zscaler.zpacloud.plugins.modules.zpa_policy_access_isolation_rule_v2.collect_all_items", return_value=([MockBox(existing_rule)], None))
        mock_client.policies.update_isolation_rule_v2.return_value = (MockBox(self.SAMPLE_RULE), None, None)
        set_module_args(provider=DEFAULT_PROVIDER, state="present", name="Test_Isolation_Rule_V2", description="Test Isolation Rule V2", action="ISOLATE")
        from ansible_collections.zscaler.zpacloud.plugins.modules import zpa_policy_access_isolation_rule_v2
        with pytest.raises(AnsibleExitJson) as result:
            zpa_policy_access_isolation_rule_v2.main()
        assert result.value.result["changed"] is True
    ...
    def test_no_change(self, mock_client, mocker):
        mocker.patch("ansible_collections.zscaler.zpacloud.plugins.modules.zpa_policy_access_isolation_rule_v2.collect_all_items", return_value=([MockBox(self.SAMPLE_RULE)], None))
        set_module_args(provider=DEFAULT_PROVIDER, state="present", name="Test_Isolation_Rule_V2", description="Test Isolation Rule V2", action="ISOLATE")
        from ansible_collections.zscaler.zpacloud.plugins.modules import zpa_policy_access_isolation_rule_v2
        with pytest.raises(AnsibleExitJson) as result:
            zpa_policy_access_isolation_rule_v2.main()
    ...
    def test_get_rule_by_id(self, mock_client, mocker):
        ...
        mock_client.policies.delete_rule.return_value = (None, None, None)
        set_module_args(provider=DEFAULT_PROVIDER, id="216199618143441990", name="Test_Isolation_Rule_V2", state="absent")
        from ansible_collections.zscaler.zpacloud.plugins.modules import zpa_policy_access_isolation_rule_v2
        with pytest.raises(AnsibleExitJson) as result:
            zpa_policy_access_isolation_rule_v2.main()
        assert result.value.result["changed"] is True
    ...
    def test_list_rules_error(self, mock_client, mocker):
        from tests.unit.plugins.modules.common.utils import AnsibleFailJson
        mocker.patch("ansible_collections.zscaler.zpacloud.plugins.modules.zpa_policy_access_isolation_rule_v2.collect_all_items", return_value=(None, "API Error"))
    ...
    def test_create_rule_error(self, mock_client, mocker):
        from tests.unit.plugins.modules.common.utils import AnsibleFailJson
        mocker.patch("ansible_collections.zscaler.zpacloud.plugins.modules.zpa_policy_access_isolation_rule_v2.collect_all_items", return_value=([], None))
        mock_client.policies.add_isolation_rule_v2.return_value = (None, None, "Create failed")
    ...
    def test_delete_rule_error(self, mock_client, mocker):
        ...
        set_module_args(provider=DEFAULT_PROVIDER, name="Test_Isolation_Rule_V2", state="absent")
        from ansible_collections.zscaler.zpacloud.plugins.modules import zpa_policy_access_isolation_rule_v2
        with pytest.raises(AnsibleFailJson):
            zpa_policy_access_isolation_rule_v2.main()
    ...
    def test_check_mode_delete(self, mock_client, mocker):
        mocker.patch("ansible_collections.zscaler.zpacloud.plugins.modules.zpa_policy_access_isolation_rule_v2.collect_all_items", return_value=([MockBox(self.SAMPLE_RULE)], None))
        set_module_args(provider=DEFAULT_PROVIDER, name="Test_Isolation_Rule_V2", state="absent", _ansible_check_mode=True)
        from ansible_collections.zscaler.zpacloud.plugins.modules import zpa_policy_access_isolation_rule_v2
        with pytest.raises(AnsibleExitJson) as result:
            zpa_policy_access_isolation_rule_v2.main()
        assert result.value.result["changed"] is True
