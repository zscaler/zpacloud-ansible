...
class TestZPAWorkloadTagGroupInfoModule(ModuleTestCase):
    SAMPLE_GROUPS = [
        {"id": "123", "name": "Production", "enabled": True},
        {"id": "456", "name": "Development", "enabled": True},
    ]

    @pytest.fixture
    def mock_client(self, mocker):
        with patch(
            "ansible_collections.zscaler.zpacloud.plugins.modules.zpa_workload_tag_group_info.ZPAClientHelper"
        ) as mock_class:
    ...
    def test_get_all_groups(self, mock_client, mocker):
        mocker.patch(
            "ansible_collections.zscaler.zpacloud.plugins.modules.zpa_workload_tag_group_info.collect_all_items",
